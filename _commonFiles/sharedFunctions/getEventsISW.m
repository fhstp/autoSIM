function [InputData, node] = getEventsISW(mot_data, mot_labels, condition, FPs, k, i, trialCnt, stepCnt, paths, events, delta, InputData)

% Decide if walking trial or SEBT trial

if contains(condition, 'star')

    % Get events via c3d events and mot file
    side = char(FPs(i));
    IC = events.(char(strcat(side,'_Foot_Strike')))(1); % first event = start
    ICi = events.(char(strcat(side,'_Foot_Strike')))(2); % last event = end
    ...
    InputData.(node).IC = IC-delta; % Vicon start frame delta to mot
    InputData.(node).TO = 0;
