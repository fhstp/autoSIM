function [path2finalModel] = ASTool(path2BaseModel, path2trc, path2SetUpFile, workingDir4AST, modelData, pose, plugInPath, trcScale)
...
% This script was made compatible for the OpenSim API4.0 since I needed
% this specific version. To make it work, there are some light changes to the Original ASTool version,
% including a quite diryt hack in createTools.m. Further this version needs
% three additonal files that are provided within a *.zip folder since I do
% not need them here directly. These are: changeXML.m, osimVec3FromArray
% and osimVec3ToArray;
...
% plugInPath = string;  path to any opensim plugIn file such as a *.dll; if not needed set to empty string '';
% trcScale = 1 or 1000; factor to calculate error.  1000 or 1; Set to 1000 if values are in mm and set to 1 if values are in m.
% ----

% Notes: make sure the initial scale setup has the names of the Static
% trial already included 2x <marker_file> for Marker Placer and Model
% Scaler. 

% Change log:
% #1 replaced the "inputAST.m with the following section between "Start Changes" and "No Further Changes" and added a line of code at the end of the script.
% #2 load_trc.m: added one line to ignore empty lines in trc files if present.
% #3 AST_core_v1.m: added option to use trc files in m or mm.
% #4 ASTool.m: Added at the end a line of code to display a finished message when ASTool is done, including processing time.
% #5 AST_core_v1.m: changed two lines of code to make searching for time points in arry more robust. 
% #6 AST_core_v1.m: added a line so that only scaler measurement sets are used from the scale file that were set as true.
% #7 Made some bigger changes for the Inverse Kinematics Tool in
% createTools, they are highlighed as section between "Start Changes" and "No Further Changes"
...
trcScale = trcScale; % 1000 or 1; Set to 1000 if values are in mm and set to 1 if values are in m.

% Set name of the finally scaled model.
name_ModelScaledAdj = 'ModelScaledMarkerAdj.osim'; 

% Set output path to the finally scaled model.
path2finalModel = fullfile(workingDir4AST, name_ModelScaledAdj); % == the final results model.
...
% Load api if user set apipath as input.
if ~isempty(plugInPath)
    opensimCommon.LoadOpenSimLibrary(plugInPath)
end
...
%% ========================== No Further Changes ==========================
%  ---------------------except for last lines of script -------------------
...
tempo_exc = toc;
tempo_minuti_exc = round(tempo_exc/60,0);


%% ========================== Some last Changes ===========================

%% Display finished message
disp([newline '<<<<<< ASTool finished after ', num2str(tempo_minuti_exc), ' minute(s). >>>>>>' newline])
