...
if Scale<0
    ...
    if Nmarkers>0
        ...
        acq = btkReadAcquisition(FullFileName);
        markers = btkGetMarkers(acq);
            fn=fieldnames(markers);
        for i=1:size(fn,1)
        tempMLabels{i}=fn{i};
            if i==1
                tempMarkers(1:size(markers.(fn{i}),1),1:3)=markers.(fn{i});
            else
                tempMarkers(1:size(markers.(fn{i}),1),((i-1)*3)+1:((i-1)*3)+3)=markers.(fn{i});
            end
        end
        Markers=tempMarkers;
    end
