...
% ----------------------------------------------------------------------
function  placeNameModel =   tibia_Rajag(dataModel, markerset, answerLeg, rightbone, TT_angle, answerNameModelTibia,...
    answerNameMarkerTibia, dataTibia, dataCalcn, dataTalus, dataToes, place, applyTibiaTorsionToJointOffset, geometryFolder, sideModel)
%% Find the muscle attachment on the tibia, calcn, talus and toes and place in a matrix
[TibiaMuscles, TibiaPlace1, TibiaNR, CalcnMuscles, CalcnPlace1, CalcnNR, ToesMuscles, ToesPlace1, ToesNR, tibiaMuscleType ] = tibia_MA(dataModel, answerLeg, rightbone);
...
for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
    if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, ['tibia_' lower(answerLeg)])
...
[dataModel, talus_old_start, calcn_old_start, toes_old_start, locationTalusRot_start, locationCalcnRot_start, ...
    locationToesRot_start]=tibia_locationInParent_rotation(dataModel, TT_angle, answerLeg, rightbone, applyTibiaTorsionToJointOffset);
...
if ~isempty(calcn_MA_BACK)
    calcn_MA_rot = (Rz_TT * calcn_MA_BACK')';
else
    calcn_MA_rot = calcn_MA_BACK;
end
toes_rot = (Rz_TT * toes_BACK')';
if ~isempty(toes_MA_BACK)
    toes_MA_rot = (Rz_TT * toes_MA_BACK')';
else
    toes_MA_rot = toes_MA_BACK;
end
...
try % gait2392 & Hamner
    jointTypes = fieldnames(dataModel.OpenSimDocument.Model.JointSet.objects);
    for j = 1 : numel(jointTypes)
        for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
            if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['ankle_' lower(answerLeg)])
                try
                    min_tibia_opensim = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame{1,1}.translation.Text);
                catch
                    min_tibia_opensim = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame.translation.Text);
                end
                break;
            end
        end
    ...
    for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
        if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, ['talus_' lower(answerLeg)])
            talusJoint = fieldnames(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint);
            min_tibia_opensim = str2num(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(talusJoint{1}).location_in_parent.Text);
            break;
        end
    end
end
...
% rotation of wrap objects
for i = 1 : wrapCnt
    distFromTop = norm(max_tibia- wrapLocations_New(i,:));
    % Highest part of the tibia - not affected
    if distFromTop <= distance_L2
        wrapLocations_New(i, :) = wrapLocations_New(i, :);
        wrapRotations_New(i, :) = wrapRotations_New(i, :);
        % The linear twist in the middle part of the bone is created
    elseif (distFromTop < distance_L3) && (distFromTop > (distance_L2))
        scalingRotVect = ((distFromTop-distance_L2)/distance_L2)*TT_angle;
        %         Rz_rot2 = [cos(scalingRotVect) -sin(scalingRotVect) 0; sin(scalingRotVect) cos(scalingRotVect) 0; 0 0 1];
        Rz_rot2 = [cos(scalingRotVect) 0 sin(scalingRotVect); 0 1 0; -sin(scalingRotVect) 0 cos(scalingRotVect)];
        wrapLocations_New(i, :) = Rz_rot2*wrapLocations_New(i, :)';

        Rz_rot2 = [1 0 0; 0 cos(-scalingRotVect) -sin(-scalingRotVect); 0 sin(-scalingRotVect) cos(-scalingRotVect)];
        wrapRotations_New(i, :) = Rz_rot2*wrapRotations_New(i, :)';

        % The lowest part of the bone is roated by the entire angle
    elseif distFromTop >= distance_L3
        Rz_TT = [cos(TT_angle) 0 sin(TT_angle);0 1 0; -sin(TT_angle) 0 cos(TT_angle)];
        wrapLocations_New(i, :) = Rz_TT*wrapLocations_New(i, :)';

        Rz_TT = [1 0 0; 0 cos(-TT_angle) -sin(-TT_angle); 0 sin(-TT_angle) cos(-TT_angle)];
        wrapRotations_New(i, :) = Rz_TT*wrapRotations_New(i, :)';
    end
end
...
%     cTibiaR = sprintf('%s_%s' ,modelName,boneName);
    boneName = 'tibia_r';
    cTibiaR = [boneName, sideModel,'.vtp'];
    placeNameTibia = sprintf('%s', direct, geometryFolder, cTibiaR);
...
%     cTibiaL = sprintf('%s_%s' ,modelName,boneName);
    boneName = 'tibia_l';
    cTibiaL = [boneName, sideModel,'.vtp'];
    placeNameTibia = sprintf('%s', direct, geometryFolder, cTibiaL);
...
if strcmp(answerLeg, rightbone) == 1;
    for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
        if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, 'tibia_r')
            if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                    .Mesh{1,1}.mesh_file.Text = cTibiaR;
            else
                    ...
                    for j = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                        ...
                        if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                            ...
                            dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                                .GeometrySet.objects.DisplayGeometry{j}.geometry_file.Text = cTibiaR;
                            break;
                        end
                    end
                else
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                        .GeometrySet.objects.DisplayGeometry.geometry_file.Text = cTibiaR;
                end
            end
            break;
        end
    ...
    for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
        if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, 'tibia_l')
            if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                    .Mesh{1,1}.mesh_file.Text = cTibiaL;
            else
                if size(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                        .GeometrySet.objects.DisplayGeometry, 2) > 1
                    for j = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                        ...
                        if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                                .GeometrySet.objects.DisplayGeometry{j}.geometry_file.Text, 'tibia')
                            dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                                .GeometrySet.objects.DisplayGeometry{j}.geometry_file.Text = cTibiaL;
                            break;
                        end
                    end
                else
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
                        .GeometrySet.objects.DisplayGeometry.geometry_file.Text = cTibiaL;
                end
            end
            break;
        end
...
if applyTibiaTorsionToJointOffset == 0
    % convert the talus data back to string
...
%         cTalusR = sprintf('%s_%s' ,modelName,boneName);
        boneName = 'talus_rv';
        cTalusR = [boneName, sideModel,'.vtp'];
        placeNameTibia = sprintf('%s', direct, geometryFolder, cTalusR);
...
%         cTalusL = sprintf('%s_%s' ,modelName,boneName);
        boneName = 'talus_lv';
        cTalusL = [boneName, sideModel,'.vtp'];
        placeNameTibia = sprintf('%s', direct, geometryFolder, cTalusL);
    ...
    if strcmp(answerLeg, rightbone) == 1;
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, 'talus_r')
                if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                        .Mesh.mesh_file.Text = cTalusR;
                else
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, 'talus_l')
                if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                        .Mesh.mesh_file.Text = cTalusL;
                else
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
...
%         cCalcnR = sprintf('%s_%s' ,modelName,boneName);
        boneName = 'calcaneous_rv';
        cCalcnR = [boneName, sideModel,'.vtp'];
        placeNameTibia = sprintf('%s', direct, geometryFolder, cCalcnR);
...
%         cCalcnL = sprintf('%s_%s' ,modelName,boneName);
        boneName = 'calcaneous_lv';
        cCalcnL = [boneName, sideModel,'.vtp'];
        placeNameTibia = sprintf('%s', direct, geometryFolder, cCalcnL);
    ...
    if strcmp(answerLeg, rightbone) == 1;
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, 'calcn_r')
                if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
                    try
                        dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                            .Mesh.mesh_file.Text = cCalcnR;
                    catch
                        dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                            .Mesh{1}.mesh_file.Text = cCalcnR;
                    end
                else
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, 'calcn_l')
                if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
                    try
                        dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                            .Mesh.mesh_file.Text = cCalcnL;
                    catch
                        dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                            .Mesh{1}.mesh_file.Text = cCalcnL;
                    end
                else
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
...
%         cToesR = sprintf('%s_%s' ,modelName,boneName);
        boneName = 'toes_r';
        cToesR = [boneName, sideModel,'.vtp'];
        placeNameTibia = sprintf('%s', direct, geometryFolder, cToesR);
...
%         cToesL = sprintf('%s_%s' ,modelName,boneName);
        boneName = 'toes_l';
        cToesL = [boneName, sideModel,'.vtp'];
        placeNameTibia = sprintf('%s', direct, geometryFolder, cToesL);
    ...
    if strcmp(answerLeg, rightbone) == 1;
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, 'toes_r')
                if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
                    try
                        dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                            .Mesh.mesh_file.Text = cToesR;
                    catch
                        dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                            .Mesh{1}.mesh_file.Text = cToesR;
                    end
                else
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1, i}.Attributes.name, 'toes_l')
                if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
                    try
                        dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                            .Mesh.mesh_file.Text = cToesL;
                    catch
                        dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                            .Mesh{1}.mesh_file.Text = cToesL;
                    end
                else
                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.VisibleObject...
...
for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
    bodyName = dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name;
    if contains(bodyName, ['calcn_' lower(answerLeg)]) || contains(bodyName, ['talus_' lower(answerLeg)]) || ...
            contains(bodyName, ['toes_' lower(answerLeg)])
%     if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['calcn_' lower(answerLeg)])
        if isfield(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}, 'attached_geometry')
            if size(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                    .Mesh, 2) > 1
                % it has more geometries ... we have to rotate all - this
                % is the fact in Lenhart model
                for j = 2 : size(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                        .Mesh, 2)
                    filename = dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                        .Mesh{1,j}.mesh_file.Text;
                    dataMesh = xml2struct(fullfile(geometryFolder, filename));

                    mesh_NUM = str2num(dataMesh.VTKFile.PolyData.Piece.Points.DataArray.Text);
                    polyText_Mesh = dataMesh.VTKFile.PolyData.Piece.Polys.DataArray{1,1}.Text;

                    [mesh_start] = coordinatesCorrection(mesh_NUM);

                    if contains(bodyName, ['calcn_' lower(answerLeg)])
                        mesh_BACK = [];
                        for h_mesh = 1:size(mesh_start,1)
                            mesh_NUM_trans = mesh_start(h_mesh,:)+ transCalcnBack + locationTalusRot_start + locationCalcnRot_start;
                            mesh_BACK = [mesh_BACK; mesh_NUM_trans];
                        end

                        mesh_rot = (Rz_TT * mesh_BACK')';

                        mesh_transformed = [];
                        for hh_mesh = 1:size(mesh_rot,1)
                            mesh_rot_trans = mesh_rot(hh_mesh,:) - (locationTalusRot_start) - (locationCalcnRot_start);
                            mesh_transformed = [mesh_transformed; mesh_rot_trans];
                        end
                    elseif contains(bodyName, ['talus_' lower(answerLeg)])

                        mesh_BACK = [];
                        for h_mesh = 1:size(mesh_start,1)
                            mesh_NUM_trans = mesh_start(h_mesh,:) + transTalusBack + locationTalusRot_start;
                            mesh_BACK = [mesh_BACK; mesh_NUM_trans];
                        end
                        ...
                        mesh_transformed = [];
                        for hh_mesh = 1:size(mesh_rot,1)
                            mesh_rot_trans = mesh_rot(hh_mesh,:) - (locationTalusRot_start) - transTalusBack;
                            mesh_transformed = [mesh_transformed; mesh_rot_trans];
                        end
                    elseif contains(bodyName, ['toes_' lower(answerLeg)])

                        mesh_BACK = [];
                        for h_mesh = 1:size(mesh_start,1)
                            mesh_NUM_trans = mesh_start(h_mesh,:) + transToesBack + locationTalusRot_start + locationCalcnRot_start + locationToesRot_start;
                            mesh_BACK = [mesh_BACK; mesh_NUM_trans];
                        end
                        ...
                        mesh_transformed = [];
                        for hh_mesh = 1:size(mesh_rot,1)
                            mesh_rot_trans = mesh_rot(hh_mesh,:) - (locationTalusRot_start) - (locationCalcnRot_start) - (locationToesRot_start);
                            mesh_transformed = [mesh_transformed; mesh_rot_trans];
                        end
                    end

                    [mesh_transformed_OpenSim]=coordinatesOpenSim(mesh_transformed);

                    % convert the tibia data back to string
                    mesh_rotated =sprintf('\t\t\t%+8.6f %+8.6f %+8.6f\n',mesh_transformed_OpenSim');
                    mesh_Repared = strrep(mesh_rotated,'+',' ');
                    % replace the generic data with the rotated bone
                    dataMesh.VTKFile.PolyData.Piece.Points.DataArray.Text = mesh_Repared;

                    % convert the struct back to xml file
                    Mesh_rotated = struct2xml(dataMesh);

                    % export - write the model as an xml  - remember to save as a vtp file
                    modelName = answerNameModelTibia;
                    boneName = [strrep(filename, '.vtp', '') [sideModel '.vtp']];
%                     cMesh = sprintf('%s_%s' ,modelName,boneName);
                    cMesh = boneName;
                    placeNameMesh = sprintf('%s', direct, geometryFolder, cMesh);
                    FID_Mesh = fopen(placeNameMesh,'w');
                    fprintf(FID_Mesh,Mesh_rotated);
                    fclose(FID_Mesh);

                    dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.attached_geometry...
                        .Mesh{1,j}.mesh_file.Text = cMesh;
                end
            end
        end
    end
...
if applyTibiaTorsionToJointOffset == 0
    for i = 1:size(CalcnMuscles,1)
        ...
        dataModel.OpenSimDocument.Model.ForceSet.objects.(muscleType){1,musclenr}.GeometryPath.PathPointSet.objects...
    ...
    for i = 1:size(ToesMuscles,1)
        ...
        dataModel.OpenSimDocument.Model.ForceSet.objects.(muscleType){1,musclenr_toes}...
...
for i = 1:size(TibiaMuscles,1)
    ...
    if contains(TibiaPlace1{i}, '{')
        ind = strfind(TibiaPlace1{i}, '{');
        tmp_place = TibiaPlace1{i}(1 : ind-1);
        dataModel.OpenSimDocument.Model.ForceSet.objects.(tibiaMuscleType{i}){1, TibiaNR(i)}...
            .GeometryPath.PathPointSet.objects.(tmp_place){1,str2double(TibiaPlace1{i}(end-1))}.location.Text = tibia_MA_OpenSim(i,:);
    else
        tmp_place = TibiaPlace1{i};
        dataModel.OpenSimDocument.Model.ForceSet.objects.(tibiaMuscleType{i}){1, TibiaNR(i)}...
            .GeometryPath.PathPointSet.objects.(tmp_place).location.Text = tibia_MA_OpenSim(i,:);
    end
%     tibiaMuscleType
%     if size(TibiaPlace1{i,1},2) == 14 ;
%         musclenr_tibia = TibiaNR(i,:);
%         string_tibia = TibiaPlace1{i,:};
%         dataModel.OpenSimDocument.Model.ForceSet.objects.(muscleType){1,musclenr_tibia}...
...
placeNameMarker = markersName;
% markerNameOut = sprintf('%s_%s', modelName, markersName);
%name and placement of the femoral bone file
% if strcmp(answerLeg,rightbone) == 1                                       %%%%%%%Changed for COMAK Workflow
