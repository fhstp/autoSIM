...
% try catch block -> either work with architecture of thelen or millard muscles
% (joints also have different positions in the OSIM-models)
%
% ------------------------------------------------------------------------
function femur_ns(dataModel, markerset, answerLeg, rightbone, FA_angle, NS_angle,answerNameModelFemur, ...
    answerNameMarkerFemur, dataFemur, place, sideModel, geometryFolder)
%% Bone vertix
...
if strcmp(answerLeg, rightbone) == 1;
    ...
    boneName = 'femur_r';               % changed by bhorsak 08/2024 to make work with autoSIM workflow
    c = [boneName, sideModel,'.vtp'];   % changed by bhorsak 08/2024 to make work with autoSIM workflow
    %placeNameFemur = sprintf('%s',direct, place, c); 
    placeNameFemur = sprintf('%s',direct, geometryFolder, c); % changed by bhorsak 08/2024 to make work with autoSIM workflow
    ...
    % c = sprintf('%s_%s' ,modelName,boneName);
    boneName = 'femur_l';               % changed by bhorsak 08/2024 to make work with autoSIM workflow
    c = [boneName, sideModel,'.vtp'];   % changed by bhorsak 08/2024 to make work with autoSIM workflow
    % placeNameFemur = sprintf('%s',direct, place, c);
    placeNameFemur = sprintf('%s',direct, geometryFolder, c); % changed by bhorsak 08/2024 to make work with autoSIM workflow
