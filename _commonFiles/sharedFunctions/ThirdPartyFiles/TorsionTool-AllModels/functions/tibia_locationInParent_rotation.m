...
if isfield(dataModel.OpenSimDocument.Model, 'JointSet')
    ...
    for j = 1 : numel(jointTypes)
        for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
            if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['ankle_' lower(answerLeg)])
                try
                    talus_old = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame{1,1}.translation.Text);
                catch
                    talus_old = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame.translation.Text);
                end
            end
            if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['subtalar_' lower(answerLeg)])
                try
                    calcn_old = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame{1,1}.translation.Text);
                catch
                    calcn_old = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame.translation.Text);
                end
            end
            if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['mtp_' lower(answerLeg)])
                try
                    toes_old = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame{1,1}.translation.Text);
                catch
                    toes_old = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame.translation.Text);
                end
            end
        end
    end
    ...
    for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
        if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['talus_' lower(answerLeg)])
            ...
            talus_old = str2num(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(talusJoint{1}).location_in_parent.Text);
        end
        if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['calcn_' lower(answerLeg)])
            calcnJoint = fieldnames(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint);
            calcn_old = str2num(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(calcnJoint{1}).location_in_parent.Text);
        end
        if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['toes_' lower(answerLeg)])
            toesJoint = fieldnames(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint);
            toes_old = str2num(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(toesJoint{1}).location_in_parent.Text);
        end
    end
end
%
...
if applyTibiaTorsionToJointOffset == 0 % original method --> torsion is applied via translation and rotation
    ...
    if isfield(dataModel.OpenSimDocument.Model, 'JointSet')
        ...
        for j = 1 : numel(jointTypes)
            for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
                if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['ankle_' lower(answerLeg)])
                    try
                        dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame{1,1}.translation.Text = num2str(locationTalusRot);
                    catch
                        dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame.translation.Text = num2str(locationTalusRot);
                    end
                        Axis_Ankle = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1, i}.SpatialTransform.TransformAxis{1, 1}.axis.Text);
                    break;
                end
            end
        end
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['talus_' lower(answerLeg)])
                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(talusJoint{1}).location_in_parent.Text = num2str(locationTalusRot);
                Axis_Ankle = str2num(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(talusJoint{1}).orientation_in_parent.Text);
                break;
            end
        end
    end
    ...
    if isfield(dataModel.OpenSimDocument.Model, 'JointSet')
        ...
        for j = 1 : numel(jointTypes)
            for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
                if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['ankle_' lower(answerLeg)])
                    dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1, i}.SpatialTransform.TransformAxis{1, 1}.axis.Text = num2str(Axis_Ankle_rot_trans);
                    break;
                end
            end
        end
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['talus_' lower(answerLeg)])
                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(talusJoint{1}).orientation_in_parent.Text = num2str(Axis_Ankle_rot_trans);
                break;
            end
        end
    end
    ...
    if isfield(dataModel.OpenSimDocument.Model, 'JointSet')
        ...
        for j = 1 : numel(jointTypes)
            for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
                if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['subtalar_' lower(answerLeg)])
                    try
                    dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame{1,1}.translation.Text = num2str(locationCalcn_transform);
                    catch
                        dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame.translation.Text = num2str(locationCalcn_transform);
                    end
                    Calcn_subtalar = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1, i}.SpatialTransform.TransformAxis{1, 1}.axis.Text);
                    break;
                end
            end
        end
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['calcn_' lower(answerLeg)])
                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(calcnJoint{1}).location_in_parent.Text = num2str(locationCalcn_transform);
                Calcn_subtalar = str2num(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(calcnJoint{1}).orientation_in_parent.Text);
                break;
            end
        end
    end
    ...
    if isfield(dataModel.OpenSimDocument.Model, 'JointSet')
        ...
        for j = 1 : numel(jointTypes)
            for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
                if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['subtalar_' lower(answerLeg)])
                    dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1, i}.SpatialTransform.TransformAxis{1, 1}.axis.Text = num2str(Calcn_subtalar_rot_trans);
                    break;
                end
            end
        end
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['calcn_' lower(answerLeg)])
                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(calcnJoint{1}).orientation_in_parent.Text = num2str(Calcn_subtalar_rot_trans);
                break;
            end
        end
    end
    ...
    if isfield(dataModel.OpenSimDocument.Model, 'JointSet')
        ...
        for j = 1 : numel(jointTypes)
            for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
                if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['mtp_' lower(answerLeg)])
                    try
                        dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame{1,1}.translation.Text = num2str(locationToes_transform);
                    catch
                        dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.frames.PhysicalOffsetFrame.translation.Text = num2str(locationToes_transform);
                    end
                    toes_mtp = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1, i}.SpatialTransform.TransformAxis{1, 1}.axis.Text);
                    break;
                end
            end
        end
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['toes_' lower(answerLeg)])
                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(toesJoint{1}).location_in_parent.Text = num2str(locationToes_transform);
                toes_mtp = str2num(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(toesJoint{1}).orientation_in_parent.Text);
                break;
            end
        end
    end
    ...
    if isfield(dataModel.OpenSimDocument.Model, 'JointSet')
        ...
        for j = 1 : numel(jointTypes)
            for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
                if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['mtp_' lower(answerLeg)])
                    dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1, i}.SpatialTransform.TransformAxis{1, 1}.axis.Text = num2str(toes_mtp_rot_trans);
                    break;
                end
            end
        end
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['toes_' lower(answerLeg)])
                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(toesJoint{1}).orientation_in_parent.Text = num2str(toes_mtp_rot_trans);
                break;
            end
        end
    end
    ...
    % end
else % alter the talus_offset rotation --> this rotates the bodies coordinate axis

    %Rotate the location
    locationTalusRot_start = (Rz_TT * talus_old_start')';
    locationCalcnRot_start = (Rz_TT * calcn_old_start')';
    locationToesRot_start = (Rz_TT * toes_old_start')';
    ...
    if isfield(dataModel.OpenSimDocument.Model, 'JointSet')
        ...
        for j = 1 : numel(jointTypes)
            for i = 1 : numel(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}))
                if contains(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1,i}.Attributes.name, ['ankle_' lower(answerLeg)])
                    Axis_Ankle = str2num(dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1, i}.frames.PhysicalOffsetFrame{1,2}.orientation.Text);

                    Axis_Ankle(2) = Axis_Ankle(2) - TT_angle;

                    dataModel.OpenSimDocument.Model.JointSet.objects.(jointTypes{j}){1, i}.frames.PhysicalOffsetFrame{1,2}.orientation.Text = num2str(Axis_Ankle);
                    break;
                end
            end
        end
        ...
        for i = 1 : numel(dataModel.OpenSimDocument.Model.BodySet.objects.Body)
            if contains(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Attributes.name, ['talus_' lower(answerLeg)])
                talusJoint = fieldnames(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint);
                Axis_Ankle = str2num(dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(talusJoint{1}).orientation.Text);

                Axis_Ankle(2) = Axis_Ankle(2) - TT_angle;

                dataModel.OpenSimDocument.Model.BodySet.objects.Body{1,i}.Joint.(talusJoint{1}).orientation.Text = num2str(Axis_Ankle);
                break;
            end
        end
    end
